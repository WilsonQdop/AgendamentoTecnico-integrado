...
export function Dashboard({
  ...
  const myOpen      = safeTickets.filter(t => t.status === 'OPEN' || t.status === 'ASSIGNED').length;
  const myProgress  = safeTickets.filter(t => t.status === 'IN_PROGRESS').length;
  const myCompleted = safeTickets.filter(t => t.status === 'COMPLETED').length;
  const myPending   = safeTickets.filter(t => t.status === 'PAYMENT_PENDING').length;
    ...
    <th className="py-3 px-2 text-center">Ver</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-zinc-800/80">
                  ...
                  {priorityTickets.map((t) => {
                    const lastComment = t.description || 'Sem comentários';
                    return (
...
<td className="py-3 px-3 text-slate-500 dark:text-zinc-400 italic text-xs max-w-[130px] truncate">{lastComment}</td>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm p-6">
            ...
            {isAdmin ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50 dark:bg-zinc-800/40 p-5 rounded-xl border ... min-h-[120px] flex flex-col justify-between">
